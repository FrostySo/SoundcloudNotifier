...
public static class CommandsExtension {
        public static string GetModuleInfo(this ModuleInfo module) {
            var commandsCountDict = new Dictionary<string, int>();
            var commandsSet = new HashSet<string>();
            foreach(var command in module.Commands) {
                commandsSet.Add(command.Name);
                if (commandsCountDict.ContainsKey(command.Name)) {
                    commandsCountDict[command.Name] += 1;
                } else {
                    commandsCountDict.Add(command.Name, 1);
                }
            }
            var moduleCommands = string.Join(", ",commandsSet.Select(command => {
                var count = commandsCountDict[command];
                if(count > 1) {
                    return $"{command} ({count} variants)";
                }
                return command;
            }));
